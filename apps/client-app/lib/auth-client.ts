...
export const forgetPassword = async (opts: ForgetPasswordOpts): Promise<AuthResult> => {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_BETTER_AUTH_URL ?? "http://localhost:3002"}/api/auth/forget-password`, {
      method: "POST",
...
export const resetPassword = async (opts: ResetPasswordOpts): Promise<AuthResult> => {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BETTER_AUTH_URL ?? "http://localhost:3002"}/api/auth/reset-password`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(opts),
      credentials: "include",
